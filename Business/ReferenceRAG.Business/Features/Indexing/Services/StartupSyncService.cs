...
public class StartupSyncService : IHostedService
{
    ...
    {
        if (!_configManager.Load().Indexing.SyncOnStartup)
        {
            _logger.LogInformation("启动同步已在配置中禁用");
        ...
        _syncTask = Task.Run(async () =>
        {
            try
            {
                await ExecuteSyncAsync(_stopping.Token);
            }
            catch (Exception ex)
            {
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var file in orphanedFiles)
        {
            ...
            try
            {
                await _pipeline.DeleteFileAsync(file.Id, file.Path, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var filePath in deletedFiles)
        {
            ...
            try
            {
                ...
                await _pipeline.DeleteFileAsync(fileRecord.Id, fileRecord.Path, cancellationToken);
