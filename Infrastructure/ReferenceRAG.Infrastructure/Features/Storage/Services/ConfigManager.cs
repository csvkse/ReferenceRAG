...
public class ConfigManager
{
    ...
    private void ApplyEnvironmentVariables(ObsidianRagConfig config)
    {
        ...
        ApplyString("REFERENCERAG_EMBEDDING_MODE", value => config.Embedding.Mode = value);
        ApplyString("REFERENCERAG_EMBEDDING_API_BASE_URL", value => config.Embedding.ApiBaseUrl = value);
        ApplyString("REFERENCERAG_EMBEDDING_API_KEY", value => config.Embedding.ApiKey = value);
        ApplyString("REFERENCERAG_EMBEDDING_MODEL_NAME", value => config.Embedding.ModelName = value);
        var embeddingDimension = Environment.GetEnvironmentVariable("REFERENCERAG_EMBEDDING_API_DIMENSION");
        if (int.TryParse(embeddingDimension, out var dimension) && dimension > 0)
            config.Embedding.ApiDimension = dimension;

        ApplyString("REFERENCERAG_RERANK_MODE", value => config.Rerank.Mode = value);
        ApplyString("REFERENCERAG_RERANK_API_BASE_URL", value => config.Rerank.ApiBaseUrl = value);
        ApplyString("REFERENCERAG_RERANK_API_KEY", value => config.Rerank.ApiKey = value);
        ApplyString("REFERENCERAG_RERANK_MODEL_NAME", value => config.Rerank.ModelName = value);
        var rerankEnabled = Environment.GetEnvironmentVariable("REFERENCERAG_RERANK_ENABLED");
        if (bool.TryParse(rerankEnabled, out var enabled))
