#!/bin/bash
#
# servicectl.sh - Control ObsidianRAG systemd service
#
...
case "$ACTION" in
    status)
        if systemctl is-active --quiet "${SERVICE_NAME}.service"; then
            echo -e "${GREEN}●${NC} ${SERVICE_NAME}.service - ObsidianRAG Knowledge Base Service"
            echo -e "   Active: ${GREEN}active (running)${NC}"
            echo ""
            echo -e "${CYAN}Recent logs:${NC}"
            journalctl -u "${SERVICE_NAME}.service" --no-pager -n 5
        else
            echo -e "${RED}●${NC} ${SERVICE_NAME}.service - ObsidianRAG Knowledge Base Service"
