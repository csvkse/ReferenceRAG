...
using Tokenizers.HuggingFace.Processors;
...
public class SystemController : ControllerBase
{
    ...
    private static int RandomValue { get; set; } = new Random().Next(1000,9999);
    ...
    {
        _logger.LogInformation("收到重启服务 API 请求");

        try
        {
            if (ServiceManager.IsService)
            {
                // 1. 直接调全局的触发器
                ServiceManager.TriggerRestart();

                // 2. 立即给前端返回成功，随后应用会自动进入优雅关闭流程

                return Ok(new RestartResponse
                {
                    Message = "服务正在重启",
                    OldProcessId = Environment.ProcessId,
                    NewProcessId = 0,
                    ProcessPath = GetServiceName() ?? "",
                    Timestamp = DateTime.UtcNow,
                    RestartMode = "Program"
                });
            }
            ...
            {
                return StatusCode(500, new { error = "重启服务失败: 必须在服务模式下才能重启！" });
            }
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "重启服务异常");
