<#
.SYNOPSIS
    Check ReferenceRAG Service Status
.DESCRIPTION
    Display detailed status information about the ReferenceRAG service.
#>
...
Write-Host "  ReferenceRAG Service Status" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
if ($service.Status -eq "Running") {
    ...
    # Find the process
    $exePath = if ($workingDir) { Join-Path $workingDir "ReferenceRAG.Service.exe" } else { $null }

    $procs = Get-Process -Name "ReferenceRAG.Service" -ErrorAction SilentlyContinue
    if (-not $procs) {
        # Try dotnet process
        $procs = Get-Process -Name "dotnet" -ErrorAction SilentlyContinue | Where-Object {
            $_.Path -like "*ReferenceRAG*"
        }
    }
