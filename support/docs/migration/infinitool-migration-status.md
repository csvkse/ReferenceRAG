...
- 迁移已合并到 `master` 并随 `v1.0.7` 发布；临时分支 `codex/infinitool-migration` 已删除。
...
测试使用本地确定性 AI 服务和独立测试目录；并未验证真实 ONNX/GPU 推理效果、模型下载、生产规模性能、Windows 服务安装或 Docker 实际运行。GitHub Actions 已于 2026-09-04 完成 `v1.0.7` 的 Windows x64 桌面端构建、打包和 Release 发布。
