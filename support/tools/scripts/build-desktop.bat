...
echo ============================================
echo   ReferenceRAG Desktop - 发布构建
echo ============================================
echo.
...
REM 设置构建参数
set PUBLISH_ARGS=-c %CONFIG% -r %RUNTIME% -o ./support/artifacts/publish/desktop
...
REM 执行构建
echo 正在构建桌面端...
dotnet publish "Host/ReferenceRAG.DesktopHost/ReferenceRAG.DesktopHost.csproj" %PUBLISH_ARGS%

if %ERRORLEVEL% equ 0 (
    echo.
    echo ============================================
    echo   桌面端构建成功！
    echo   输出目录: %cd%\support\artifacts\publish\desktop
    echo   可执行文件: ReferenceRAG.DesktopHost.exe
    echo ============================================

    REM 记录构建日志
    echo [%date% %time%] 桌面端构建成功 - %CONFIG% %RUNTIME% >> "%~dp0..\..\artifacts\logs\desktop-build.log"
) else (
    echo.
    echo ============================================
    echo   桌面端构建失败！错误代码: %ERRORLEVEL%
    echo ============================================

    REM 记录失败日志
    echo [%date% %time%] 桌面端构建失败 - 错误代码: %ERRORLEVEL% >> "%~dp0..\..\artifacts\logs\desktop-build.log"
)
