...
if "%EXE_PATH%"=="" (
    echo 错误: 未找到 ReferenceRAG.WebHost.exe
    pause
    exit /b 1
)
...
if %ERRORLEVEL% equ 0 (
    echo.
    echo 卸载完成！
    echo [%date% %time%] 服务卸载成功 >> "%~dp0..\..\artifacts\logs\service.log"
) else (
    echo.
    echo 卸载失败，错误代码: %ERRORLEVEL%
    echo [%date% %time%] 服务卸载失败 - 错误代码: %ERRORLEVEL% >> "%~dp0..\..\artifacts\logs\service.log"
)
